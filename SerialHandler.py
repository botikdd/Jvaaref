...
class MessageConverter:
    ...
    def PIDS(kp,ki,kd,tf):
        if type(kp)==float and type(ki)==float and type(kd)==float and type(tf)==float:    
            return "#PIDS:%.2f;"%kp+"%.2f;"%ki+"%.2f;"%kd+"%.2f;"%tf+";\r\n"
        else:
            return ""
    ...
    def checkWaiters(self,f_response):
        ...
        if l_key in self.Waiters:
            ...
            for waiter in l_waiters:
                waiter.set()
    def addWaiter(self,f_key,f_objEvent):
        if f_key in self.Waiters:
            obj_events_a=self.Waiters[f_key]
            obj_events_a.append(f_objEvent)
        else:
            obj_events_a=[f_objEvent]
            self.Waiters[f_key]=obj_events_a
    def deleteWaiter(self,f_key,f_objEvent):
        if f_key in self.Waiters and f_objEvent in self.Waiters[f_key]:
            l_obj_events_a=self.Waiters[f_key]
            l_obj_events_a.remove(f_objEvent)            
    def stop(self):
        self.Run=False
    def start(self):
        self.Run=True
...
class SerialHandler:
    def __init__(self,f_device_File='/dev/ttyACM0',f_history_file='historyFile.txt'):
        self.serialCon=serial.Serial(f_device_File,460800,timeout=1)#460800
